...
public static class XunitTestCollectionRunnerTests
{
	...
	public static class Run
	{
		...
		{
			var testCase = TestData.XunitTestCase<TestClassWithCtorThrowingOrderer>(nameof(ClassUnderTest.Passing));
			var runner = new TestableXunitTestCollectionRunner(testCase);
			...
			Assert.Collection(
				...
				msg =>
				{
					...
					Assert.Collection(
						failure.Messages,
						msg => Assert.Equal($"Collection-level test class orderer '{typeof(MyCtorThrowingOrderer).FullName}' for test collection '{typeof(TestCollectionWithCtorThrowingOrderer).FullName}' threw during construction", msg),
		...
		[CollectionDefinition]
		[TestClassOrderer(typeof(MyCtorThrowingOrderer))]
		public class TestCollectionWithCtorThrowingOrderer
		{ }

		[Collection(typeof(TestCollectionWithCtorThrowingOrderer))]
		...
		class MyCtorThrowingOrderer : ITestClassOrderer
		{
